...
casper.test.begin('Test the cycle for cloze questions', function(test) {
    ...
    casper.then(function () {
        ...
        test.assertTruthy(response.id, "Repsonse must have contained an id");
    });
    ...
    casper.then(function () {
        casper.thenOpen("http://localhost:8888/question/" + response.id, function () {
            casper.waitForSelector('#code', function () {
                ...
                test.assertExists('#sendButton', "Expect the sendButton here");
    ...
    casper.then(function () {
        casper.waitForSelector('td.resultcell', function () {
            test.assertTextExists(textToExpect, 'Inserted Text must be shown correctly!');
    ...
    casper.then(function () {
        casper.waitForSelector('#theAnswer', function () {
            test.assertTextExists(insertedTextToExpect, 'Dialog text must be shown correctly!');
        });
    });
